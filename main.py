...
class Segment(pyglet.sprite.Sprite):
    ...
    def __init__(self, x, y):
        ...
        # Set height, width
        self.image = pyglet.Surface([segment_width, segment_height])
...
while not done:
    ...
    for event in pyglet.event.get():
        ...
        if event.type == pyglet.KEYDOWN:
            if event.key == pyglet.K_LEFT:
                x_change = (segment_width + segment_margin) * -1
                y_change = 0
            if event.key == pyglet.K_RIGHT:
                x_change = (segment_width + segment_margin)
                y_change = 0
            if event.key == pyglet.K_UP:
                x_change = 0
